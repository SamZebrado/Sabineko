...
const runningNorthNo1Jobs = new Map();
const NorthNo1_SEND_GUARD_PATH = path.join(ROOT, 'state', 'northno1_send_guard.json');
...
function setNorthNo1WorkflowStatus(paths, patch) {
  updateStatus(paths, (s) => {
    s.running = false;
...
function startCapture(paths, forceLogin) {
  ...
  if (runningNorthNo1Jobs.has(paths.paperId)) {
...
function runNorthNo1Send(paths, body = {}) {
...
function runNorthNo1NewChat(paths, body = {}) {
...
function performNorthNo1Send(paths, body = {}) {
  if (runningNorthNo1Jobs.has(paths.paperId)) {
    throw new Error(`NorthNo1 fetch/parse already running for ${paths.paperId}`);
  }
  ...
  const throttle = shouldGuardSend
    ? checkNorthNo1SendThrottle(paths, cfg, force)
  ...
  let output;
  try {
    output = runNorthNo1Send(paths, body);
  } catch (err) {
    setNorthNo1WorkflowStatus(paths, {
      currentStep: 'error',
  ...
  setNorthNo1WorkflowStatus(paths, body.composeOnly
    ? {
...
function runNorthNo1FetchParse(paths, body = {}) {
...
function executeRequest(paths, request = {}) {
  ...
  if (action === 'capture.run') {
    if (runningJobs.has(paths.paperId) || runningNorthNo1Jobs.has(paths.paperId)) {
  ...
  if (action === 'northno1.new_chat') {
    return {
      ok: true,
      action,
      output: runNorthNo1NewChat(paths, params),
      northno1StatusPath: relToRoot(path.join(paths.paperStateDir, 'northno1_bridge_status.json'))
    };
  }
  ...
  if (action === 'northno1.send' || action === 'northno1.compose') {
    ...
    return { action, composeOnly: Boolean(body.composeOnly), ...performNorthNo1Send(paths, body) };
  }
  ...
  if (action === 'northno1.fetch_parse') {
    ...
    startNorthNo1FetchParse(paths, params);
    return {
      ok: true,
      started: true,
      action,
...
function startNorthNo1FetchParse(paths, body = {}) {
  if (runningNorthNo1Jobs.has(paths.paperId)) {
    throw new Error(`NorthNo1 fetch/parse already running for ${paths.paperId}`);
  }
  if (runningJobs.has(paths.paperId)) {
  ...
  runningNorthNo1Jobs.set(paths.paperId, child);
  ...
  child.on('close', (code) => {
    runningNorthNo1Jobs.delete(paths.paperId);
...
function checkNorthNo1SendThrottle(paths, cfg, force) {
  if (force) {
...
function readSendGuard() {
  if (!fs.existsSync(NorthNo1_SEND_GUARD_PATH)) {
    return {
      inFlight: false,
      currentPaperId: null,
  ...
  return safeReadJson(NorthNo1_SEND_GUARD_PATH) || {
    inFlight: false,
    currentPaperId: null,
    lastSentPaperId: null,
    lastSentAt: null
...
function writeSendGuard(guard) {
  fs.mkdirSync(path.dirname(NorthNo1_SEND_GUARD_PATH), { recursive: true });
  fs.writeFileSync(NorthNo1_SEND_GUARD_PATH, JSON.stringify(guard, null, 2), 'utf8');
}
  ...
  if (scopedPaperMatch) {
    ...
    if (req.method === 'GET' && subPath === '/status') {
      sendJson(res, 200, {
        ...
        running: runningJobs.has(paths.paperId) || runningNorthNo1Jobs.has(paths.paperId)
      });
      return;
    }
    ...
    if (req.method === 'POST' && subPath === '/actions/northno1-send') {
      const body = await readBody(req);
      try {
        sendJson(res, 200, performNorthNo1Send(paths, body));
      } catch (err) {
    ...
    if (req.method === 'POST' && subPath === '/actions/northno1-fetch-parse') {
      ...
      startNorthNo1FetchParse(paths, body);
      sendJson(res, 200, {
        ok: true,
        started: true,
        paperId: paths.paperId,
